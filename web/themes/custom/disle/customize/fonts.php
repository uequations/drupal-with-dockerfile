...
function disle_render_option_font(){
    $fonts = disle_fonts();
    $output = '';
...
function disle_typography_font_styles($option, $selectors) {
    $output = $selectors . ' {';
...
function disle_typography_enqueue_google_font($font) {
 
    if($font && $font != "---"){
        if(array_search($font, array_keys(disle_fonts())) > 17){
...
function disle_links_typography_font($json){
    $links_fonts = '';
    $customize = (array)json_decode((string)$json, true);
    if(isset($customize['font_family_primary']) && $customize['font_family_primary']){
        $links_fonts .= disle_typography_enqueue_google_font($customize['font_family_primary']);
    }
    if(isset($customize['font_family_second']) && $customize['font_family_second']){
        $links_fonts .= disle_typography_enqueue_google_font($customize['font_family_second']);
    }
    return $links_fonts;
}

function disle_options_patterns(){
    $output = '';
    $file_path = \Drupal::service('extension.list.theme')->getPath('disle');
